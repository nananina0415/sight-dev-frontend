...
const CATEGORIES = ["CLUB", "ACADEMIC", "EXTERNAL"] as const;
...
export default function ReservationForm({
  ...
  const handleSubmit = () => {
    if (!canSubmit || !selectedRoom || !selectedRoom.isSelectable) return;
    onSubmit({
      category,
  ...
  return (
    <div className={styles["reservation-form"]}>
      ...
      <div className={styles["form-field"]}>
        ...
        {selectedRoom ? (
          <>
            <div className={styles["selected-room-badge"]}>
              🚪 {selectedRoom.name} ({selectedRoom.capacity}명)
            </div>
            {!selectedRoom.isSelectable && (
              <div className={styles["room-unavailable-warning"]}>
                ⚠️ 이 방은 일정을 등록할 수 없습니다.
                <br />
                410호 또는 405호를 선택해주세요.
              </div>
            )}
          </>
        ) : (
          <div className={styles["no-room-hint"]}>
            ↑ 위 약도에서 동아리 방(410호 또는 405호)을 클릭하여 선택해주세요
          </div>
        )}
      </div>
